...
import aspect.rest.models.{ProjectResult, ProjectUserResult}
...
class RestGetProjectTest extends TestBase {
  ...
  it should "return project successfully" in {
    ...
    //assert
    result shouldBe ProjectResult(project.id, project.name, ProjectUserResult(user.id, user.name), project.createdAt)
    //cleanup
  ...
  it should "return 401 (Unauthorized) CREDENTIALS_MISSING if authorization token is not defined" in {
    //act
    val result = Rest.getProject(ProjectId.gen).toErrorResult
    //assert
  ...
  it should "return 401 (Unauthorized) CREDENTIALS_REJECTED if authorization token is not correct" in {
    //act
    val result = Rest.getProject(ProjectId.gen, SessionToken.gen).toErrorResult
    //assert
